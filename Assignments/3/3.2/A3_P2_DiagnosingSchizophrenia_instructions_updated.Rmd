...
test_results <- 
  ...
  mutate(
    ...
    bt_prob  = predict(m_bt, new_data = test_baked, type = "prob") %>% 
      pull() # I left all pulls empty as it didn't work with any argument I tried
  )
...
cv_folds <- vfold_cv(df_test2, v = 10, repeats = 10, strata = diagnosis, group=uID)
...
predict_log <- function(split, rec, model) {
  ...
  baked_test <- bake(opskrift2, testing(split))
  tibble(
    actual = baked_test$diagnosis,
    predicted = predict(model, new_data = baked_test) %>% pull(.pred_class),
    prop_diag =  predict(model, new_data = baked_test, type = "prob") %>% pull(.pred_diagnosis),
    prop_no_diag =  predict(model, new_data = baked_test, type = "prob") %>% pull(`.pred_no_diagnosis`)
  ) 
}
