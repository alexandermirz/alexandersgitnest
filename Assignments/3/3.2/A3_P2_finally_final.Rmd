...
##### PLEASE NOTE, ESPECIALLY YOU, KENNETH #####

## If you look for the code of random forest, svm and boost tree, you won't find them here. Because we used the same lines of code for all models with just changing a type and an engine of the model.

# N.B. Kenneth's message from the last class received :-)

Data&packages, preprocessing data, models and their performance matrics
```{r}
### data&package ###
pacman::p_load(tidyverse, tidymodels, ggplot2, groupdata2, parsnip, randomForest, C50, psych)
df <- read.csv(file="~/alexandersgitnest/Assignments/3/3.1/demo_art_pitch_danish.csv")

data <- df
data

# deleting unnecessary columns (uID preserved to do partitioning according to both id and diagnosis)
#data[,c(1,2,3,5,6,8,10,11,12,13,14,15,20,28,30,31,32,34)] <- NULL
data <- df[,c(4,7,9, 16, 17,18,19, 21,22,23,24,25,26,27,29, 33, 35)]
...
test_best_results
sd(test_best_results$lr_best_prob)
...
## creating folds
cv_folds <- vfold_cv(df_train, v = 10, repeats = 100, strata = diagnosis, group=uID)
