...
test_data = read_pitch("Study1D0S126T6_f0.txt")


 # when you've created a function that works, you can
# pitch_data = 
#     list.files(path = "~/alexandersgitnest/Assignments/3/3.1/data/Pitch/",pattern = ".txt") %>% ## NB replace with your path to the files
#     purrr::map_df(read_pitch)
# pitch_data %>% filter(subject==101)
...
demo_data <- 
  read.csv("DemographicData.csv", header = TRUE, sep= ";")

pitch_data <- 
  read.csv("pitch_data.csv", header = TRUE, sep = ",")


pitch_data1 <- pitch_data %>% 
  filter(study==3)#|study==2|study==3|study==4)
...
demo_data1 <- demo_data %>% 
  ...
  # 
  # demo1$Diagnosis <- ifelse(demo1$Diagnosis =="Control", "0", "1")
  # names(demo1)[3]= "diagnosis"
  # names(demo1)[1]="study"
  # names(demo1)[4]="subject"
  # merged <- merge(study1, demo1, by=c("study", "subject"))
  # ?colnames
  # 
...
pitch_data_merged <- 
  merge(pitch_data1, demo_data1, by=c("study", "diagnosis", "subject"))

# then importing duration data
art_data <- 
...
# Testing whether the problem with merging is the same
art_data1 <- art_data %>% 
  filter(study==1|study==2|study==3|study==4)

demo_art <- merge(art_data1, demo_data1, by=c("study", "diagnosis", "subject"))
