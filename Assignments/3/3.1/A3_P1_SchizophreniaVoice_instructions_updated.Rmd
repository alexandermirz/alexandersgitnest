...
```{r}
pacman::p_load(tidyverse, readr, psych, lme4, ggplo2)
...
names(demo_data)[3]= "diagnosis"
names(demo_data)[1]="study"
names(demo_data)[4]="subject"
demo_data$diagnosis <- 
  ifelse(demo_data$diagnosis =="Control", "0", "1")

# Missing values when merging: Study 3
# merging demo_data1 and pitch_data1:
pitch_demo_all <- 
  merge(demo_data, pitch_data,
        by=c("subject",
             "study",
...
#reordering columns
pitch_demo_all <- pitch_demo_all[,c(2,3, 1, 14, 4:13, 15:18)]

#merging demo/pitch and art data:
demo_art_pitch <- 
  merge(pitch_demo_all, art_data,
...
# Now we save them
write.csv(demo_art_pitch, file = "demo_art_pitch_all.csv")
```
...
#in order to be able to scale the variable pause duration, we need to get rid of all 0 in npause
demo_art_pitch2 <- filter(demo_art_pitch2, npause!=0)
...
# }
# 
# new_df <-as.data.frame(cbind("iqr", "prop", "speech_rate", "pause_duration"))
# for (i in 1:4){
#   data = demo_art_pitch2
#   inside_df = subset(data,study == i) %>%
#     scale(data$iqr) %>% scale(mean)
#   new_df = filter
#   var_df = merge(demo_art_pitch2, filterr)
#   return(var_df)
#   
#  
# }
...
pacman::p_load(tidyverse, readr, psych, lme4, lmerTest)
...
demo_art_pitch2$pause_duration <- 
  scale(demo_art_pitch2$pause_duration) #finishing the scaling

demo_art_pitch2 <- mutate(demo_art_pitch2, nor_scaled_pause_duration=scale(pause_duration))
...
## pitch variability
m_pvar_scal <-
  lmer(scaled_iqr ~ diagnosis+ (1|uID), demo_art_pitch2, REML = FALSE)
summary(m_pvar_scal)

## proportion of spoken time
m_spokent_scal <- 
  lmer(scaled_prop_phon~diagnosis+(1|uID), demo_art_pitch2, REML = FALSE)
summary(m_spokent_scal)

## speech rate
m_speechr_scal <- 
  lmer(scaled_speech_rate~diagnosis+(1|uID), demo_art_pitch2, REML = FALSE)
summary(m_speechr_scal)

## pause duration

m_pausedur_scal <- 
  lmer(nor_scaled_pause_duration~diagnosis+(1|uID), demo_art_pitch2, REML = FALSE)
...
scaled <- function(var){
  a <- c(NULL)
  variable_dataframe <- as.data.frame(a)
  for(i in 1:4){
    dat=demo_art_pitch2 %>% 
    ...
    variable_scaled=dat %>% 
      select(var) %>% 
      scale()
    data_prep <- merge(variable_scaled, i)
    variable_dataframe <- rbind(variable_dataframe, data_prep)
  }
  return(variable_dataframe)
  
}

iqr_scaled_study <- scaled("iqr")

demo_art_pitch2$propor_spokt=demo_art_pitch2$phonationtime..s./demo_art_pitch2$dur..s.

propor_spokt_scaled <- scaled("propor_spokt")

speech_rate_scaled_study <- scaled("speechrate..nsyll.dur.")

scaled_pause_duration<- scaled("pause_duration")
...
demo_art_pitch2 <- 
  mutate(demo_art_pitch2,
         ...
         propor_spokt_scaled$propor_spokt,
         scaled_pause_duration$pause_duration)

### buillding models based on scaled pr. study

## pitch variability
m_pvar_scal_study <-
  lmer(iqr_scaled_study$iqr ~ diagnosis+(1|uID), demo_art_pitch2, REML = FALSE)
summary(m_pvar_scal_study)

CI_iqr <- confint(m_pvar_scal_study, level = 0.90)
CI_iqr


## proportion of spoken time
m_spokent_scal_study <- 
  lmer(propor_spokt_scaled$propor_spokt~diagnosis+(1|uID), demo_art_pitch2, REML = FALSE)
summary(m_spokent_scal_study)

CI_prop_spokt <- confint(m_spokent_scal_study, level = 0.90)
CI_prop_spokt

## speech rate
m_speechr_scal <- 
  lmer(speech_rate_scaled_study$speechrate..nsyll.dur.~diagnosis+(1|uID), demo_art_pitch2, REML = FALSE)
summary(m_speechr_scal)

CI_speech_rate <- confint(m_speechr_scal, level = 0.90)
CI_speech_rate

## pause duration
m_pausedur_scal <- 
  lmer(scaled_pause_duration$pause_duration~diagnosis+(1|uID), demo_art_pitch2, REML = FALSE)
summary(m_pausedur_scal)

CI_pause_duration <- confint(m_pausedur_scal, level = 0.90)
CI_pause_duration
####model with random effect for study
m_pvar_ranstud <- 
  lmer(scaled_iqr~diagnosis)
