...
# demo_art_pitch2$pause_duration[!is.finite(demo_art_pitch2$pause_duration)] <- 0 #Replacing all inf & NaN with 0
demo_art_pitch2$pause_duration <- subset(demo_art_pitch2, !is.finite(demo_art_pitch2$pause_duration))
demo_art_pitch2$pause_duration <- 
  scale(demo_art_pitch2$pause_duration) #finishing the scaling

filter(!is.finite(demo_art_pitch2$pause_duration))

demo_art_pitch2 %>% filter(pause_duration, is.na())
### buillding models
...
m_pausedur_scal <- 
  lmer(pause_duration~diagnosis+(1|uID), demo_art_pitch2, REML = FALSE)
