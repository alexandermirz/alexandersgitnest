...
read_pitch <- function(filename) {
    ...
    string=str_split(filename, "D",
                     simplify = T)
    study=str_extract(string[1],
                      "\\d")
    # Diagnosis:
    diag=str_split(string[2],
                   "S",
                   simplify =T)
    diagnosis=diag[1]
    # Subject: 
    subj=str_split(diag[2],
                   "T",
                   simplify = T)
    subject=subj[1]
    # Trial: 
    tri=str_split(subj[2],
                  "[[:punct:]]",
                  simplify = T)
    trial=tri[1]
    #Getting the data: 
    data=read.delim(paste0("~/alexandersgitnest/Assignments/3/3.1/data/Pitch/",
                           filename))
...
demo_data1$diagnosis <- 
  ifelse(demo_data1$diagnosis =="Control",
         "0",
         "1")
...
clean_df <- function(data) {
    ## parse filename to extract study, diagnosis, subject and trial
    string=str_split(data$soundname,
                     "D",
                     simplify = T)
    study=str_extract(string[,1],
                      "\\d")
...
# Now we save them
write.csv(demo_art_pitch,
          file = "demo_art_pitch.csv")
```
...
m_spokent_scal_study <- 
  lmer(propor_spokt_scaled$propor_spokt~diagnosis+(1|uID),
       demo_art_pitch2,
       REML = FALSE)
...
# proportion of spoken time
m_spoken_gender <- 
  lmer(propor_spokt_scaled$propor_spokt~diagnosis + (1|uID) + (1|Gender),
       demo_art_pitch2,
       REML = FALSE)
...
# pause duration
m_pausedur_gender <- 
  lmer(scaled_pause_duration$pause_duration~diagnosis + (1|uID) + (1|Gender),
       demo_art_pitch2,
       REML = FALSE)
summary(m_speechrate_gender)

m_pausedur_gender1 <- 
  lmer(scaled_pause_duration$pause_duration~diagnosis + (1|uID),
       demo_art_pitch2,
       REML = FALSE)
summary(m_speechrate_gender1)
