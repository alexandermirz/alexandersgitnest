...
```{r}
demo_train <- 
  read_csv("~/alexandersgitnest/Assignments/1/Data/demo_train.csv")
LU_train <- 
  read_csv("~/alexandersgitnest/Assignments/1/Data/LU_train.csv")
token_train <- 
  read_csv("~/alexandersgitnest/Assignments/1/Data/token_train.csv")
```
...
```{r}
dat1 <- 
  select(LU_train, SUBJ, VISIT, MOT_MLU, CHI_MLU)
dat2 <- 
  select(demo_train, Diagnosis, Ethnicity, Gender, Age, ADOS, MullenRaw, ExpressiveLangRaw, Socialization, SUBJ, VISIT)
dat3 <- 
  select(token_train, types_MOT, types_CHI, tokens_MOT, tokens_CHI)
```
...
asd_full_df <- 
  cbind(dat1, dat3)
asd_full_df <- 
  merge(asd_full_df, dat2) #20 rows are removed from initial df demo_train... 
...
visit_1 <- 
  asd_full_df %>%
  filter(asd_full_df$VISIT == 1) #new df with only 1st visit
...
asd_full_df <- 
  ...
  rename(
    ADOS1 = ADOS, 
    MR = MullenRaw,
    ExLaRaw = ExpressiveLangRaw,
    Socializ = Socialization
    ) #Renaming critial columns 

#asd_full_df <- 
  #subset(asd_full_df, select = -c(VISIT)) #removing the visit column

asd_full <- 
  merge(visit_1, asd_full_df) #Merged version of the dataframes with data only from 1st visit
...
```{r}
asd_full$SUBJ <- 
  rownames(asd_full[0]) #with the help of indexing, the children are anonymized by using the rowname as identifier

asd_full$Gender <-
  ifelse(asd_full$Gender == 1, yes = "F", no = "M") #changing the gendertypes 1 and 2 to F and M

asd_full$Diagnosis <- 
  ifelse(asd_full$Diagnosis == "A", yes = "ASD", no = "TD") #changing diagnosis types from A and B to ASD and TD 

###Final save####
write.csv(asd_full, file = "asd_full", row.names = FALSE)
##################
```
...
```{r}
filter(asd_full_df, CHI_MLU>2.7)#1
filter(asd_full, CHI_MLU<1.5)#2

asd_full_df %>%
  filter(!complete.cases(asd_full_df[1:15]))#3 (Using complete cases)
...
```{r}
#previously, the visit column has been removed, so the functions above are rerun and some held back to avoid removal of visit column. 
asd_full_df %>%
  filter(VISIT==6) %>%
  arrange(desc(tokens_CHI)) #using desc to arrange in descending order.
...
asd_full_df %>%
  filter(Diagnosis=="A") %>%
  select(CHI_MLU, tokens_CHI, CHI_MLU) #nothing happens
...
asd_full_df <- mutate(asd_full_df, mean(asd_full_df$tokens_CHI)) #Adding the new column #1

visit_tokens <- vector("numeric") #creating and empty vector #2

for (i in asd_full_df$VISIT){
  sub=filter(asd_full_df, VISIT==i)
